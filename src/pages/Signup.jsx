  ...
  const handleSuccess = async (user, tokens) => {
    ...
    if (!user.isVerified) {
      ...
      if (user.leetcodeUsername) {
        localStorage.setItem('tempLeetcodeUsername', user.leetcodeUsername);
      }
      navigate('/verify');
    } else {
      if (user.leetcodeUsername) {
      ...
      >
        ...
        <form className="space-y-4 text-left" onSubmit={handleSignup}>
          <div>
            ...
            <Input icon={FiUser} placeholder="John Doe" type="text" value={name} onChange={e => setName(e.target.value)} required />
          </div>
          <div>
          ...
          <div>
            ...
            <Input icon={FiUser} placeholder="leet_master" type="text" value={leetcodeUsername} onChange={e => setLeetcodeUsername(e.target.value)} required />
          </div>
